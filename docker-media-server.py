...
---------------------------
Initializes and runs the Flask application with SocketIO using eventlet for WebSockets.
...
# Import application components *after* monkey patching
...
# You can use an environment variable or default to 'development'
...
if __name__ == '__main__':
    ...
    # Check if running as executable and cloudflared.exe exists
    cloudflared_path = None
    
    # Check for cloudflared in the current directory
    script_dir = os.path.dirname(os.path.abspath(__file__))
    for cloudflared_name in ['cloudflared', 'cloudflared.exe']:
        potential_path = os.path.join(script_dir, cloudflared_name)
    ...
    if cloudflared_path:
        try:
            # In Docker, use environment variable instead of prompting
            ...
            if use_tunnel == 'y':
                ...
                # Function to read stdout/stderr and find the URL
    ...
    # --- End Cloudflare Tunnel Integration ---
    ...
    try:
        ...
        if hasattr(socket, 'SOL_SOCKET') and hasattr(socket, 'SO_KEEPALIVE'):
            ...
            try:
                ...
                # This block is no longer needed as we rely on socketio.run()
                pass
        ...
        # Ensure tunnel process is terminated when the server stops
        if tunnel_process:
